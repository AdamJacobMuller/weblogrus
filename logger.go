...
type Middleware struct {
	...
	Name   string
	Before func(*logrus.Entry, *web.Request, string) *logrus.Entry
	After  func(*logrus.Entry, web.ResponseWriter, time.Duration, string) *logrus.Entry
...
func (m *Middleware) ServeHTTP(rw web.ResponseWriter, r *web.Request, next web.NextMiddlewareFunc) {
	if m.Before == nil {
...
type BeforeFunc func(*logrus.Entry, *web.Request, string) *logrus.Entry

// AfterFunc is the func type used to modify or replace the *logrus.Entry after
// calling the next func in the middleware chain
type AfterFunc func(*logrus.Entry, web.ResponseWriter, time.Duration, string) *logrus.Entry

// DefaultBefore is the default func assigned to *Middleware.Before
func DefaultBefore(entry *logrus.Entry, req *web.Request, remoteAddr string) *logrus.Entry {
...
func DefaultAfter(entry *logrus.Entry, res web.ResponseWriter, latency time.Duration, name string) *logrus.Entry {
	return entry.WithFields(logrus.Fields{
		"status":      res.StatusCode(),
		"text_status": http.StatusText(res.StatusCode()),
		"took":        latency,
